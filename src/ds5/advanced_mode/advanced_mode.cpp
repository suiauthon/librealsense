...
namespace librealsense
{
    ...
    void cs_advanced_mode_base::cs_advanced_mode_init(std::shared_ptr<hw_monitor> hwm, cs_sensor* depth_sensor)
    {
        ...
        _preset_opt = std::make_shared<cs_advanced_mode_preset_option>(*this,
                                                                       *_depth_sensor,
                                                                       option_range{ 0,
                                                                                     RS2_RS400_VISUAL_PRESET_COUNT - 1,
                                                                                     1,
    ...
    bool cs_advanced_mode_base::is_enabled() const
    {
        return *_enabled;
    }
    ...
    void cs_advanced_mode_base::get_amp_factor(STAFactor* ptr, int mode) const
    {
        *ptr = _amplitude_factor_support ? get<STAFactor>(advanced_mode_traits<STAFactor>::group, nullptr, mode) :
               []() { STAFactor af; af.amplitude = 0.f; return af; }();
    }
    ...
    void cs_advanced_mode_base::get_laser_power(laser_power_control* ptr) const
    {
        if (supports_option(*_depth_sensor, RS2_OPTION_LASER_POWER))
        {
            ptr->laser_power = _depth_sensor->get_option(RS2_OPTION_LASER_POWER).query();
    ...
    void cs_advanced_mode_base::get_laser_state(laser_state_control* ptr) const
    {
        if (supports_option(*_depth_sensor, RS2_OPTION_EMITTER_ENABLED))
        {
            ptr->laser_state = static_cast<int>(_depth_sensor->get_option(RS2_OPTION_EMITTER_ENABLED).query());
    ...
    void cs_advanced_mode_base::get_depth_exposure(exposure_control* ptr) const
    {
        get_exposure(*_depth_sensor, ptr);
    }

    void cs_advanced_mode_base::get_depth_auto_exposure(auto_exposure_control* ptr) const
    {
        get_auto_exposure(*_depth_sensor, ptr);
    }

    void cs_advanced_mode_base::get_depth_gain(gain_control* ptr) const
    {
        if (supports_option(*_depth_sensor, RS2_OPTION_GAIN))
        {
            ptr->gain = _depth_sensor->get_option(RS2_OPTION_GAIN).query();
    ...
    void cs_advanced_mode_base::get_depth_auto_white_balance(auto_white_balance_control* ptr) const
    {
        if (supports_option(*_depth_sensor, RS2_OPTION_ENABLE_AUTO_WHITE_BALANCE))
        {
            ptr->auto_white_balance = static_cast<int>(_depth_sensor->get_option(RS2_OPTION_ENABLE_AUTO_WHITE_BALANCE).query());
    ...
    void cs_advanced_mode_base::get_color_exposure(exposure_control* ptr) const
    {
        if (*_color_sensor)
        {
            get_exposure(*(*_color_sensor), ptr);
    ...
    void cs_advanced_mode_base::get_color_auto_exposure(auto_exposure_control* ptr) const
    {
        if (*_color_sensor)
        {
            get_auto_exposure(**_color_sensor, ptr);
    ...
    void cs_advanced_mode_base::set_amp_factor(const STAFactor& val)
    {
        if (_amplitude_factor_support)
        {
            set(val, advanced_mode_traits<STAFactor>::group);
    ...
    void cs_advanced_mode_base::set_laser_power(const laser_power_control& val)
    {
        if (val.was_set)
            _depth_sensor->get_option(RS2_OPTION_LASER_POWER).set(val.laser_power);
    }

    void cs_advanced_mode_base::set_laser_state(const laser_state_control& val)
    {
        if (val.was_set)
            _depth_sensor->get_option(RS2_OPTION_EMITTER_ENABLED).set((float)val.laser_state);
    }
    ...
    void cs_advanced_mode_base::set_depth_exposure(const exposure_control& val)
    {
        if (val.was_set)
            set_exposure(*_depth_sensor, val);
    }

    void cs_advanced_mode_base::set_depth_auto_exposure(const auto_exposure_control& val)
    {
        if (val.was_set)
            set_auto_exposure(*_depth_sensor, val);
    }

    void cs_advanced_mode_base::set_depth_gain(const gain_control& val)
    {
        if (val.was_set)
            _depth_sensor->get_option(RS2_OPTION_GAIN).set(val.gain);
    }

    void cs_advanced_mode_base::set_depth_auto_white_balance(const auto_white_balance_control& val)
    {
        if (val.was_set)
            _depth_sensor->get_option(RS2_OPTION_ENABLE_AUTO_WHITE_BALANCE).set(float(val.auto_white_balance));
    }

    void cs_advanced_mode_base::set_color_exposure(const exposure_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_exposure value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_auto_exposure(const auto_exposure_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_auto_exposure value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_backlight_compensation(const backlight_compensation_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_backlight_compensation value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_brightness(const brightness_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_brightness value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_contrast(const contrast_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_contrast value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_gain(const gain_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_gain value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_gamma(const gamma_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_gamma value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_hue(const hue_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_hue value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_saturation(const saturation_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_saturation value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_sharpness(const sharpness_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_sharpness value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_white_balance(const white_balance_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_white_balance value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_auto_white_balance(const auto_white_balance_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_auto_white_balance value! Color sensor not found.");
    ...
    void cs_advanced_mode_base::set_color_power_line_frequency(const power_line_frequency_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_power_line_frequency value! Color sensor not found.");
    ...
    std::vector<uint8_t> cs_advanced_mode_base::send_receive(const std::vector<uint8_t>& input) const
    {
        auto res = _hw_monitor->send(input);
        if (res.empty())
        {
    ...
    uint32_t cs_advanced_mode_base::pack(uint8_t c0, uint8_t c1, uint8_t c2, uint8_t c3)
    {
        return (c0 << 24) | (c1 << 16) | (c2 << 8) | c3;
    }

    std::vector<uint8_t> cs_advanced_mode_base::assert_no_error(ds::fw_cmd opcode, const std::vector<uint8_t>& results)
    {
        if (results.size() < sizeof(uint32_t)) throw std::runtime_error("Incomplete operation result!");
        auto opCodeAsUint32 = pack(results[3], results[2], results[1], results[0]);
        if (opCodeAsUint32 != static_cast<uint32_t>(opcode))
    ...
    std::vector<uint8_t> cs_advanced_mode_base::encode_command(ds::fw_cmd opcode,
                                                               uint32_t p1,
                                                               uint32_t p2,
                                                               uint32_t p3,
                                                               uint32_t p4,
    ...
    rs2_rs400_visual_preset cs_advanced_mode_preset_option::to_preset(float x)
    {
        return (static_cast<rs2_rs400_visual_preset>((int)x));
    }
    ...
    bool cs_advanced_mode_preset_option::is_enabled() const
    {
        return true;
    }

    const char* cs_advanced_mode_preset_option::get_description() const
    {
        return "Advanced-Mode Preset";
    }

    const char* cs_advanced_mode_preset_option::get_value_description(float val) const
    {
        try {
            return rs2_rs400_visual_preset_to_string(to_preset(val));
        }
    ...
    uint16_t cs_advanced_mode_preset_option::get_device_pid(const cs_sensor& sensor) const
    {
        auto str_pid = sensor.get_info(RS2_CAMERA_INFO_PRODUCT_ID);
        uint16_t device_pid{};
        std::stringstream ss;
    ...
    firmware_version cs_advanced_mode_preset_option::get_firmware_version(const cs_sensor& sensor) const
    {
        return firmware_version(_ep.get_info(RS2_CAMERA_INFO_FIRMWARE_VERSION));
    }
}
